...
;; on: 7-Nov-2018
;; at: 16:09:01.779367 UTC
;; owner: carl [script library member who can update
;; this script]
;; =================================================
Red [
	Title: "MakeDoc - A Red Document Formatter"
	Author: "Carl Sassenrath"
	Original-Date: 10-Mar-2007 ;10-Jan-2005
	Redapted: "Toomas Vooglaid"
	Redate: 13-Nov-2018 ;7-Nov-2018
	File: %makedoc.red
	Purpose: {
		This is adaptation for Red from original REBOL MakeDoc2. 
		See http://www.rebol.net/docs/makedoc.html
	}
	Usage: {
		...
		If you use Red GUI console the output file will be displayed in
		...
		These are supported:
			...
			do/args %makedoc.red %document.txt

			do/args %makedoc.red 'load-only
...
commands: [
	newline
	here: (debug ["---PARSE:" copy/part here any [find here newline tail here]])
...
;   "<<" reserved
	...
	| #"=" [
		  #"=" output (emit output trim code) ; /auto toomasv
		| "image" image
		...
		| "url" url text-line (emit url reduce [lnk text])
	]

	;-- Primary implied paragraph types:
	| example (emit code detab code); trim/auto toomasv
	| paragraph (
...
url: [some space copy lnk to space]
...
set 'scan-doc func [str /options block] [
	...
	parse detab str rules    ;parse/all toomasv 2018-11-07
	if verbose [
		n: 1
...
set 'gen-html func [
	...
	if not no-template [
		...
		if not template [template: trim default-template]  ; /auto toomasv
	]
	...
	prior-cmd: none
	catch [   ; toomasv added 8-Nov-2018
		forskip doc 2 [
...
<td align="center">
<span class="tail"><a href="http://www.red-lang.org">MakeDoc2 by Red</a> - $date</span>
</td></tr></table>
</body></html>
}
...
emit-url: func [spec][
	; Emit link. Spec = [link text].
	emit [
		join {<br><a href="} [(spec/1) {">}]
		spec/2 
		</a>
	]
]
...
do-makedoc: has [in-view? file msg doc] [
	...
	in-view?: system/console/gui? ; toomasv
	...
	if not file [
		either in-view? [
			file: request-file ;/only ;toomasv
		][
			file: ask "Filename? "
